<script>
	...
	async function submitRegister(event) {
		...
		try {
			...
			// Si on a réussit, on stocke le token de validation
			if (result.result?.validationToken) {
				validationToken = result.result.validationToken;
			}	
		} catch (error) {
			alert(error);
		} finally {
...
<section class="min-h-screen flex items-center justify-center p-6 font-sora overflow-hidden relative">
	...
	<div class="relative max-w-5xl w-full mt-10 mb-32 z-10">
		...
		>
			{#if validationToken}
				<div class="w-full h-[420px] flex items-center justify-center bg-yellow-300">
					<a
						href="/validate?validationToken={validationToken}"
						class="text-2xl font-black uppercase border-4 border-black px-8 py-6 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:bg-green-400 transition-all hover:translate-x-1 hover:translate-y-1 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
					>
